    ...
    <div x-show="!submitted" class="print:!block">
        <div class="max-w-[900px] mx-auto px-6 pt-11 pb-24 print:pt-0 print:pb-0">
            ...
            @if(isset($isClosed) && $isClosed)
                <div
                    class="bg-white border border-[#e8ddc4] rounded-[22px] shadow-[0_18px_48px_rgba(11,42,91,0.10)] p-[clamp(24px,4vw,44px)] mt-6 text-center">
                    <div
                        class="mx-auto w-[72px] h-[72px] bg-[#f8e5e5] text-[#d93838] rounded-full flex items-center justify-center mb-5">
                    ...
                    <p class="text-[#6b7280] text-[15px] md:text-[16px] leading-relaxed max-w-lg mx-auto mb-8">
                        Mohon maaf, periode pendaftaran DPDRI <i>AWARDS</i> 2026 telah resmi berakhir pada tanggal <strong
                            class="text-[#10131a]">{{ \Carbon\Carbon::parse($tanggalPenutupan)->locale('id')->translatedFormat('d F Y') }}</strong>
                        pukul {{ \Carbon\Carbon::parse($tanggalPenutupan)->format('H:i') }} WIB.
                    </p>
                    <a href="{{ route('landing') }}"
                ...
                validate(step) {
                    ...
                    if (step === 2) {
                        ...
                        if (penghargaanInvalid) {
                            this.errs.penghargaanList = "Harap lengkapi uraian, tahun, beserta lampirkan bukti dukung untuk penghargaan yang ditambahkan.";
                            hasErr = true;
                    ...
                    try {
                        ...
                        xhr.onload = () => {
                            ...
                            try {
                                ...
                                if (xhr.status === 413) {
                                    result = { success: false, message: 'Ukuran total file/berkas terlalu besar melebihi batas server. Mohon perkecil ukuran file Anda.' };
                                } else if (xhr.status === 419) {
                                    result = { success: false, message: 'Sesi Anda telah berakhir. Silakan muat ulang (refresh) halaman ini.' };
                                } else {
                                    result = { success: false, message: 'Respons server tidak valid, Silakan hubungi admin.' };
                                }
                            }
